...
class DatabaseNoteBook: NoteStorageProtocol {
    ...
    func add(_ note: Note) {
        ...
        guard let newNote = NSEntityDescription.insertNewObject(forEntityName: "NoteEntity", into: bgContext) as? NoteEntity else {
            return
        }
        ...
        if let color = note.color.noteColorParse() {
            if let newColor = NSEntityDescription.insertNewObject(forEntityName: "ColorEntity", into: bgContext) as? ColorEntity {
